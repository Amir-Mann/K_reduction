...
class Network:
    ...
    def test(self, down, up, label, soundness=True):
        ...
        for i in range(self._last_layer_id):
            ...
            res = self.evalAffineExpr(layer=i + 1, back_substitute=self.FULL_BACKSUBSTITUTION,
                                      sound=soundness)
            vec_to_ret.append(res)
        res = self.evalAffineExpr(diffMatrix, back_substitute=self.FULL_BACKSUBSTITUTION,
                                  sound=soundness)
        return (res > 0).all(), vec_to_ret
