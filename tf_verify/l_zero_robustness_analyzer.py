...
class LZeroRobustnessAnalyzer:
    ...
    def analyze(self):
        ...
        covering_sizes, S = self.__load_covering_sizes_and_aproximate_s(p_vector)
        strategy, A = self.__choose_strategy(covering_sizes, S, w_vector)
        ...
        self.__release_workers(strategy, normalization_buckets)
    ...
    def __estimate_p_w_and_scores(self, plot_p_vector=False, plot_w_vector=False):
        ...
        if plot_p_vector:
            ...
            plt.plot(x_axis, p_vector, label='filtered and repaired')
        ...
        if plot_w_vector:
            x_axis = list(range(sampling_lower_bound, sampling_upper_bound + 1))
            plt.plot(x_axis, average_time_vector, label='data')
            plt.plot(x_axis, w_vector, label='filtered')
            plt.xlabel('number of pixels')
    ...
    def __release_workers(self, strategy, normalization_buckets):
    # TODO: send gpu workers more data(covering_sizes, running_times, buckets)
        for worker_index, gpu_worker in enumerate(self.__gpu_workers):
            gpu_worker.send((self.__image, self.__label, strategy, worker_index,
                             len(self.__gpu_workers), normalization_buckets))
    ...
    def __wait_while_iterating_covering(self, analyzer_start_time, statistics_by_size,
        ...
        while done_count < len(self.__gpu_workers):
            if self.__timeout != -1 and time.time() - analyzer_start_time >= self.__timeout:
                print('\nTimed out!')
                print('*******************************************************')
                self.__stop_workers()
    ...
    def __handle_adversarial(self, pixels, adversarial_example, adversarial_label, plot_adversarial=False):
        self.__stop_workers()
        ...
        if plot_adversarial:
            plt.figure(1)
            plt.subplot(211)
            plt.imshow(np.array(self.__image).reshape(28, 28), cmap="gray")
            plt.figure(1)
            plt.subplot(212)
            plt.imshow(np.array(adversarial_example).reshape(28, 28), cmap="gray")
            plt.show()
