...
parser.add_argument('--k_low', type=int, default=1, help='lowest k to sample')
parser.add_argument('--k_high', type=int, default=784, help='highest k to sample')
parser.add_argument('--k_delta', type=int, default=1, help='delta between ks to sample')
parser.add_argument('--num_samples', type=int, default=50, help='number of samples from each k that get sampled')
parser.add_argument('--stats_file', type=str, help='exported stats will be added to image_bounds_stats/netname/stats_file.json')
# amir omer noa END
...
if dataset=='acasxu':
    ...
    if config.stats_file is None:
        json_file_name = f"image_bounds_stats/{modified_netname}/{config.dataset}"
    else:
        json_file_name = f"image_bounds_stats/{modified_netname}/{config.stats_file}"
        ...
        if is_correctly_classified == True:
            ...
            if domain == 'gpupoly' or domain =='refinegpupoly':
                ...
                if domain.endswith("poly"):
                    ...
                    for K1 in range(config.k_low, config.k_high, config.k_delta):
                        NUM_ATTEMPTS_AT_FINDING_FAILING_ORIGINS = 50000
                        samples_per_k = []
                        for repeat_of_K1 in range(config.num_samples):
                            sample_found = False
                        ...
                        # adding k list to file
                        k_obj = {"dataset": config.dataset, "network": config.netname, "image": i, "k" : K1, "samples": samples_per_k}
