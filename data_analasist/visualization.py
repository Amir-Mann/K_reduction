...
def visualize_d_and_y_funcs(x_funcs, y_funcs, file_names):
  ...
  for i, func_tuple_for_y in enumerate(y_funcs):
    ...
    if GRID_PRESENTATION:
      if num_rows > 1:
        my_plotter = axs[position_breakdown(i, COLUMNS)[0], position_breakdown(i, COLUMNS)[1]] # yes I tried unpacking
      else:
    ...
    for j, color, x_func_tup in zip(range(1000), mcolors.BASE_COLORS, x_funcs):
        ...
        lst_r_squared_xfunc.append((r_squared, f"X: {x_func_name}, Y: {y_func_name}")) # TODO !! CHANGE TO ONLY NUMBER
